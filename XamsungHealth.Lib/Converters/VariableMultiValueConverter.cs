...
{
	//Adapted from https://github.com/xamarin/XamarinCommunityToolkit/
	public class VariableMultiValueConverter : VariableMultiValueConverter<object>
	{
	...
	/// </summary>
	public class VariableMultiValueConverter<TObject> : MultiValueConverterExtension, IMultiValueConverter
	{
		enum Modes
		{
			Bool,
		...
		public object Convert(object[]? values, Type targetType, object? parameter, CultureInfo culture)
		{
			...
			{
				return EvaluateCondition(false);
			}
			...
			{
				MultiBindingCondition.Any => EvaluateCondition(count >= 1),
				MultiBindingCondition.None => EvaluateCondition(count == 0),
				MultiBindingCondition.Exact => EvaluateCondition(count == Count),
				MultiBindingCondition.GreaterThan => EvaluateCondition(count > Count),
				MultiBindingCondition.LessThan => EvaluateCondition(count < Count),
